...
def train():
    ...
    return render_template("train.html" , loss =loss, acc = acc, top_acc = top_acc)
...
def predict():
    ...
    for i in range(len(windowss)):
        ...
        w_json.append(windowss[i]['width'])
        h_json.append(windowss[i]['height'])
        crop_img = edit(image_list3[0],windowss[i]['width'],windowss[i]['height']).crop(windowss[i]['posx'],windowss[i]['posy'])
        rotate_img = edit(image_list3[0],windowss[i]['width'],windowss[i]['height']).rotate(crop_img, windowss[i]['rotation'])
        names = image_list3[0].split("/")
        directory = 'static/datasets/crop/' + windowss[i]['name'] + '/'+ windowss[i]['name'] + '_' + names[1]
        cv2.imwrite(directory, rotate_img)
        #print(directory)
        img_name = names[1]
    ...
    ng = len(files) - c
    print(status)
    return render_template("predict.html", original = image_list3[0] ,files = files, result = result, ok = c, ng = ng, status=status, x_json = x_json , y_json = y_json, w_json = w_json, h_json = h_json )
